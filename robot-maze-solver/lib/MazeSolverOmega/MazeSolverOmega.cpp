#include "MazeSolverOmega.h"

MazeSolverOmega::MazeSolverOmega(BLEController& ble, LineSensor& lineSensor, MotorController& motors, RGBLEDController& rgbLED, UltrasonicSensor& usSensor)
    : ble(ble), lfSensor(lineSensor), motors(motors), rgbLED(rgbLED), usSensor(usSensor) {
}

void MazeSolverOmega::update() {
    // read sensors
...
bool MazeSolverOmega::isGoalReached() const {
    return reachedGoal;
}

void MazeSolverOmega::resetAll() {
    resetIslandCheck();
    rightWallBlockedCounter = 0;
    currentMode = Mode::NORMAL;
    reachedGoal = false;
...
void MazeSolverOmega::handleRightOpeningOffset() {
    motors.driveForward();
    delay(550);
}

void MazeSolverOmega::handleUltrasonicPositionOffset() {
    motors.driveForward();
    delay(350);
}

void MazeSolverOmega::handleBackoff() {
    motors.driveBackward();
    delay(100);
}

// Omega Version Helper Methods

void MazeSolverOmega::recordRectangleSegment() {
...
bool MazeSolverOmega::validateRectangleIsland() {
...
void MazeSolverOmega::resetIslandCheck() {
    leftTurnCounter = 0;
